...
>
> The `bmad-clickup-dev-implement` skill found a GitHub Personal Access Token prefix embedded in `git remote -v` output for the current repo. Continuing would mean every subsequent `git push` reads the leaked token, and any forked clone of this repo retains it.
>
...
5. Validate the result is a non-empty alphanumeric string (letters and digits only, no spaces or special characters). If the result fails validation, emit the following standard error block and stop:
   ...
   The `bmad-clickup-dev-implement` skill could not extract a ClickUp task ID from the input you provided.
