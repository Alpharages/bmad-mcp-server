...
After the permission gate passes, the skill resolves PRD, architecture, and epics paths **client-side** (the AI runs the 3-layer cascade against the client project root — `.bmadmcp/config.toml [docs]` → BMAD `_bmad/config.toml` chain → `planning-artifacts/` default — rather than calling a server tool, since the MCP server may be remote and cannot see the project's files). Unlike `bmad-clickup-create-story`, these files are **optional** for bug creation — the skill warns if any file is missing but **does not abort**.
