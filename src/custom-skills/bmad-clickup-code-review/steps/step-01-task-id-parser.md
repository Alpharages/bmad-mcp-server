...
>
> The `bmad-clickup-code-review` skill found a GitHub Personal Access Token prefix embedded in `git remote -v` output for the current repo. Continuing would mean every subsequent `git push` reads the leaked token.
>
> **What to do:**
>
...
5. Validate the result is a non-empty alphanumeric string. If validation fails, emit the error block and stop:
   ...
   The `bmad-clickup-code-review` skill could not extract a ClickUp task ID from the input you provided.
