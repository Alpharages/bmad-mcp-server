...
> **Inherited context:** `{task_id}` and `{supplied_base_url}` are declared in step-01's frontmatter and are available here.
...
3. **Best-effort-epic.** If the parent epic is absent or its fetch fails, emit the warning block, leave `{epic_task_id}` and `{epic_name}` as empty strings, and continue.
4. **Status advisory.** If `{task_status}` is not a QA-family status (`ready for qa`, `in qa`, `qa`, `qa in progress`, `ready for testing`, `in testing`, `to test`), emit the status advisory block — but do NOT stop. QA may legitimately be triggered on tasks in other statuses.
...
5. If `{task_status}` is not in the QA-family set, emit the status advisory block (non-fatal) and continue.
...
8. Extract and store: `{epic_task_id}` ← `task_id`, `{epic_name}` ← `name`. If this call failed, emit the epic-context-unavailable warning and leave both as empty strings.
...
The `bmad-clickup-qa` skill called `getTaskById` with task ID `{task_id}` but received an error response.
...
### Advisory block — task not in QA status

```
⚠️ **Task status advisory — task is not currently in a QA status**

Task `{task_id}` ({task_name}) has status `{task_status}`, which is not a QA-family status.

**Impact:** QA will proceed. If you intended to QA a different task, stop now and re-invoke with the correct task ID.
```
...
**Impact:** QA will use the task description and planning artifacts only — no epic-level context.
```
...
Proceeding to step 3 (QA notes extraction).
```
