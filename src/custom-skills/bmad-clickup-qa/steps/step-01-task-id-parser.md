...
>
> The `bmad-clickup-qa` skill found a GitHub Personal Access Token prefix embedded in `git remote -v` output for the current repo.
>
> **What to do:**
>
...
6. Validate the result is a non-empty alphanumeric string. If validation fails, emit the error block and stop:
   ...
   The `bmad-clickup-qa` skill could not extract a ClickUp task ID from the input you provided.
