...
>
> The `bmad-clickup-create-epic` skill requires `CLICKUP_MCP_MODE=write`. The current mode does not register `createTask`.
>
...
3. **If any file is missing, emit the following error block and stop:**
   ...
   The `bmad-clickup-create-epic` skill requires the following files to exist:
