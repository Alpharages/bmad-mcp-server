...
The `clickup-qa` skill found neither a "## QA / Testing Notes" section, a "## Human QA Notes" section, nor an "## Acceptance Criteria" section in task `{task_id}` ({task_name}).

**What to do:** This task was likely not created via `clickup-create-story` (which writes these sections), or the description was edited to remove them. Add the QA sections / acceptance criteria to the ClickUp task, or re-create the story with `clickup-create-story`, then re-invoke QA.
```
