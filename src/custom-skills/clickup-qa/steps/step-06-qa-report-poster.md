...
>
> The `clickup-qa` skill requires `CLICKUP_MCP_MODE=write` (so `addComment` is registered) to post the QA report to ClickUp. The full report is shown above for you to copy in manually.
