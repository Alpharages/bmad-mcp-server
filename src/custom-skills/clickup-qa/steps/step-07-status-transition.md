...
>
> The `clickup-qa` skill requires `CLICKUP_MCP_MODE=write` to transition task status. The `updateTask` tool is not available.
>
...
>
> The `clickup-qa` skill called `getListInfo` for list `{list_id}` but could not find a matching status for verdict `{qa_verdict}`.
>
