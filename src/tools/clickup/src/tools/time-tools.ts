...
import { getAllTeamMembers, taskIdSchema } from "../shared/utils";
...
export function registerTimeToolsRead(server: McpServer) {
  server.tool(
    ...
    {
      task_id: taskIdSchema.optional().describe("Optional task ID to filter entries. If not provided, returns all user's time entries."),
...
export function registerTimeToolsWrite(server: McpServer) {
  server.tool(
    ...
    {
      task_id: taskIdSchema.describe("The task ID to book time against, exactly as given"),
