...
- **Epic goal:** Stand up the `lore-memory-mcp` server with a Postgres + pgvector schema, a Docker compose for local dev, and the minimal MCP tool surface (store-lesson, query-lessons) so the remainder of the Lore Platform build-order has a working memory backend to target.
...
The pilot epic is scoped to a single component of the Lore Platform (`lore-memory-mcp`) with a bounded deliverable: a Postgres + pgvector schema, a Docker compose for local dev, and two MCP tools (`store-lesson`, `query-lessons`). Estimated at ≥3 stories over the 2-week Sprint 1 window (2026-04-27 → 2026-05-10), which matches EPIC-5's "at least 3 stories in one epic" outcome and sits inside the "≤~6 stories / ≤~2 weeks" guardrail in this story's Dev Notes.
